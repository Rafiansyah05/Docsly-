...
export function PremiumModal({ compact = false, className = '' }: { compact?: boolean; className?: string }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger
        render={
          <Button variant="outline" className={`h-9 ${compact ? 'w-9 px-0' : 'px-4 gap-2'} bg-[#2563EB] hover:bg-[#1D4ED8] text-white border-0 font-medium transition-all rounded-[8px] ${className}`}>
      ...
      <DialogContent className="sm:max-w-[750px] rounded-2xl bg-white p-0 overflow-hidden">
        {/* Header Section */}
        <div className="bg-slate-50 px-8 py-10 border-b border-slate-200 flex flex-col items-center text-center">
          ...
          <DialogTitle className="text-2xl font-bold text-slate-900 mb-2">Kerja Lebih Cepat, Hasil Lebih Maksimal</DialogTitle>
        ...
        <div className="grid md:grid-cols-2 gap-0">
          ...
          <div className="p-8 border-b md:border-b-0 md:border-r border-slate-200 flex flex-col bg-white">
            ...
            <p className="text-[14px] text-slate-500 mt-1 h-10">Pilihan cerdas untuk pelajar & pekerja individu.</p>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Kuota cerdas AI 50.000 kata', 'Ditenagai model unggulan GPT-4o', 'Ekspor dokumen bersih (tanpa watermark)', 'Akses ke semua template standar'].map((feature, i) => (
            ...
            <Button onClick={() => handleUpgrade('Docsly Pro')} disabled={isLoading} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold border-0 h-12 rounded-xl transition-colors text-[14px]">
              Pilih Paket Pro
            </Button>
          </div>
          ...
          <div className="p-8 relative flex flex-col bg-white">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[14px] text-slate-500 mt-1 h-10">Kekuatan penuh untuk profesional & tim.</p>
              </div>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Kuota AI tanpa batas (Unlimited)', 'Jalur prioritas saat server sibuk', 'Fitur kolaborasi real-time (Segera)', 'Dukungan bantuan prioritas'].map((feature, i) => (
                <li key={i} className="flex items-start gap-3 text-[14px] text-slate-700">
            ...
            <Button onClick={() => handleUpgrade('Docsly Business')} disabled={isLoading} className="w-full bg-[#2E75B6] hover:bg-[#245f96] text-white font-semibold border-0 h-12 rounded-xl transition-colors text-[14px]">
