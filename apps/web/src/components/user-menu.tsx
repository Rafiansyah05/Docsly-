...
export function UserMenu({ user, profile }: { user?: User; profile?: any }) {
  const fallbackName = profile?.nama_lengkap || user?.email || 'User';
  const avatarUrl = profile?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(fallbackName)}&background=random`;
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="relative h-8 w-8 rounded-full outline-none focus-visible:ring-2 focus-visible:ring-slate-400">
        <img src={avatarUrl} alt="Avatar" className="rounded-full border border-slate-200 w-full h-full object-cover" />
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-64 p-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-lg" align="end">
        <DropdownMenuGroup>
          <DropdownMenuLabel className="font-normal p-2">
            <div className="flex flex-col space-y-1">
              <p className="text-sm font-semibold leading-none text-slate-900 dark:text-zinc-100">{profile?.nama_lengkap || 'Pengguna'}</p>
              <p className="text-xs leading-none text-slate-500 dark:text-zinc-400">{user?.email || 'user@example.com'}</p>
            </div>
          </DropdownMenuLabel>
        </DropdownMenuGroup>
