...
export function TiptapEditor({ documentId, initialContent, initialTitle, workspaceId }: TiptapEditorProps) {
  ...
  const executePageDelete = (nodes: { from: number; to: number }[]) => {
    ...
    nodes.sort((a, b) => b.from - a.from).forEach(({ from, to }) => {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-slate-50">
      ...
      <div className="flex-1 flex flex-row min-h-0 overflow-hidden">
        <div 
          className="flex-1 overflow-y-auto pb-4 md:pb-8 flex flex-col items-center bg-slate-200"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl max-w-sm w-full mx-4 overflow-hidden border border-zinc-200">
            ...
            <div className="bg-slate-50 px-5 py-3 flex justify-end gap-2 border-t border-slate-100">
              <button 
                onClick={() => { setShowDeleteModal(false); setPendingDeleteNodes([]); }}
                className="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors"
              >
                Batal
              </button>
              <button 
                onClick={() => executePageDelete(pendingDeleteNodes)}
      ...
      {contextMenu && contextMenu.visible && (
        <div 
          className="fixed z-50 min-w-[200px] bg-white rounded-lg shadow-xl border border-slate-200 py-1"
