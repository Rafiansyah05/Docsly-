...
let ExportService = class ExportService {
    async generatePdf(title, html, pageSettings, layout, pageRanges) {
        let browser = null;
        try {
      ...
      `;
            ...
            const pxToMm = (px) => `${(px * 25.4 / 96).toFixed(3)}mm`;
            const topMargin = layout?.top != null ? pxToMm(layout.top) : '25.4mm';
